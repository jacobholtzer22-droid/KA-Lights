...
export default function FeatureCards() {
  ...
  return (
    <Section labelledBy={copy.heading ? 'features-heading' : undefined} label={copy.heading ? undefined : 'Why permanent lighting'}>
        ...
        <ul className="mt-12 grid gap-6 sm:grid-cols-2 lg:[grid-template-columns:repeat(auto-fit,minmax(14rem,1fr))]">
          {cards.map((card) => (
            <li key={card.title} className="relative rounded-card border border-line bg-surface p-8">
              ...
              <span aria-hidden="true" className="flex h-12 w-12 items-center justify-center rounded-site border border-line text-accent">
