      ...
      <ol className="mt-10 border-t border-line">
        {services.map((s, i) => (
          <li key={s.slug} className="border-b border-line">
            <Link href={`/services/${s.slug}`} className={`group grid gap-3 py-7 transition-colors md:grid-cols-12 md:items-baseline md:gap-8 ${focusable}`}>
              <span className="font-heading text-step--1 font-semibold tabular-nums text-accent-ink md:col-span-1">{String(i + 1).padStart(2, '0')}</span>
              <h3 className="font-heading text-step-2 font-semibold text-ink group-hover:text-accent-ink md:col-span-4">{s.name}</h3>
